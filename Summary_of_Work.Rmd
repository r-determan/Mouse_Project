...
accuracy_table <- function(result,title = "",  all_results){
  zero_rule_acc <- ifelse(sum(result$true == 0)>sum(result$true == 1), 
...
plot_cm(result = data.frame(true = factor(testing$open), pred = factor(pred_logis)), title = "Logistic Reg w/ PCA")
all_results <- accuracy_table(data.frame(true = factor(testing$open),
                                         pred = factor(pred_logis)),
                              title = "Logistic Reg w/ PCA", 
...
all_results <- accuracy_table(result,
                              title = "Neural Net", 
                              all_results = all_results)
...
plot_cm(result = result, title = "Incorrect RNN")
all_results <- accuracy_table(result,
                              title = "Incorrect RNN", 
                              all_results = all_results)

rm(arframe, xdata, epochs, istrain, model, n, n_neurons, xrnn, y_true, ydata, zero_rule_acc, lagm, cm, history, result, kpred)
```
...
all_results <- accuracy_table(result,
                              title = "Correct RNN w/ all neurons", 
                              all_results = all_results)
...
```{r}
model <- keras_model_sequential () %>%
  layer_lstm(n_lags*2, input_shape = list(n_lags , n_neurons), return_sequences = TRUE) %>% 
...
plot_cm(result = result, title = "LSTM")
all_results <- accuracy_table(result,
                              title = "LSTM", 
                              all_results = all_results)
...
plot_cm(result, "LSTM All Neurons")
accuracy_table(result, "LSTM All Neurons", all_results)
