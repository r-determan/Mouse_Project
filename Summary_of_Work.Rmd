...
# Final Model 
## Bidirectional LSTM All Neurons
```{r}
epochs <- 100
n_neurons <- ncol(data_clean)-1
n_lags <- 5
...
plot_cm(result, "Bidir LSTM All Neurons")
all_results <- accuracy_table(result, train_y = ydata[istrain],"Bidir LSTM All Neurons", all_results)
```
...
for (mouse in unique(bz_all$df)[1:3]){
  result <- NULL
  print(mouse)
...
plot_cm(result = result, title = "Logistic Reg w/ PCA")
all_results <- accuracy_table(result, training$open,
                              title = "Logistic Reg w/ PCA (chunk)", 
...
<<<<<<< HEAD
  set.seed(1)
  train <- seq(1, round(.6*nrow(comp)))
=======

  train <- seq(1, round(.8*nrow(comp)))
>>>>>>> ff6a4f1901307d36243f25b9a4a101b75207895c
  training <- comp[train,]
  testing <- comp[-train,]
...
plot_cm(result, "Bidir LSTM 25 Neurons")
all_results <- accuracy_table(result,train_y = ydata[istrain], "Bidir LSTM 25 Neurons", all_results)
