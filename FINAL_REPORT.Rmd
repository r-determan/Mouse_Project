...
plot_cm <- function(result, title = ""){
  ...
  plot_confusion_matrix(cm$`Confusion Matrix`[[1]],
    ...
    )) + labs(title = title) + theme(plot.title.position = "plot", plot.margin = unit(c(0,0,0,0), "cm"))
}
...
ggplot(data = res, aes(x =mouse))+
  geom_point(mapping = aes(y = zero_rule_acc, color = "Baseline"), size = 1.5)+
  geom_point(mapping = aes(y = BILSTM, color = "BI-LSTM"), size = 1.5)+
  geom_point(mapping = aes(y = LSTM, color = "LSTM"), size = 1.5)+
